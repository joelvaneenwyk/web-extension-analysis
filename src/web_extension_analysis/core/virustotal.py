...
def scan_url(url):
    ...
    if response["response_code"] == 1:
        ...
        if finalresp["response_code"] == 1:
            print(
                "{0}/{1} - {2}".format(
                    finalresp["positives"], finalresp["total"], finalresp["permalink"]
                )
            )
        else:
            return [
                False,
                "Reached maximum rate limit for virustotal api! If you are using your own key, please wait a minute and try again",
            ]
    else:
        return [
            False,
...
def scan_domain(domain):
    ...
    try:
        ...
        if response["response_code"] == 1:
            ...
            return [
                False,
                "Either rate limited or something else went wrong while getting domain report from virustotal",
            ]
...
def domain_batch_scan(domains):
    ...
    if core.virustotal_api != "":
        ...
        for _domain in domains:
            core.updatelog(
                "Skipping virustotal domain scan for {0}. Reason: No virustotal api added!".format(
                    _domain
                )
            )
