...
def init_settings():
    ...
    if not os.path.isfile(core.settings_file):
        ...
        try:
            ...
            core.updatelog(
                "Error {0} encountered while getting settings file from github... Please download a clean version of ExtAnalysis from github.".format(
                    str(e)
                )
            )
        ...
        try:
            ...
            if core.reports_path != new_results_dir:
                if os.path.isdir(new_results_dir):
                    ...
                    core.updatelog(
                        "Invalid results_directory_path specified in settings.json! using default path: {0}".format(
                            core.reports_path
                        )
                    )
            ...
            if lab_dir != "" and lab_dir != core.lab_path:
                if os.path.isdir(lab_dir):
                    ...
                    core.updatelog(
                        "Invalid lab_directory_path specified in settings.json! using default lab path: {0}".format(
                            core.lab_path
                        )
                    )
            ...
            core.updatelog(
                "Something went wrong while reading settings file. Error: " + str(e)
            )
            logging.error(traceback.format_exc())
...
def path_changed(old_path, new_path):
    ...
    for report in reports["reports"]:
        if "<reports_path>" in report["report_directory"]:
            core.updatelog(
                "[Updating reports index] Chainging <report_index> to: " + old_path
            )
            report["report_directory"] = report["report_directory"].replace(
                "<reports_path>", old_path
            )
...
def changedir(newpath):
    ...
    if os.path.isdir(newpath):
        ...
        try:
            ...
            ws.close()
            core.updatelog(
                "File successfully updated! rewriting variables and fixing old paths..."
            )
...
def update_settings_batch(settings_dict):
    ...
    try:
            ...
            try:
                if type(settings[the_setting]) is bool:
                    ...
                    if str(settings_dict[the_setting]).lower() == "true":
                        ...
                        core.updatelog(
                            "Set the value of {0} to True successfully".format(
                                the_setting
                            )
                        )
                        # set update type
                        ...
                        core.updatelog(
                            "Set the value of {0} to False successfully".format(
                                the_setting
                            )
                        )
                        # set update type
                        ...
                        core.updatelog(
                            "Invalid value: {1} for setting {0}".format(
                                the_setting, str(settings_dict[the_setting])
                            )
                        )
                        # set update type
                        if update_type == "":
        ...
        try:
            ...
            ws.close()
            core.updatelog(
                "Settings written to file successfully! Restart ExtAnalysis for them to take effect"
            )
        except Exception as e:
            core.updatelog(
                "Error {0} occured while writing settings.json file".format(str(e))
            )
            logging.error(traceback.format_exc())
            return "0"
        return update_type
