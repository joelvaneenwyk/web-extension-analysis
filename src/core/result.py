...
class createresult:
            ...
            for folder in dirlist:
                ...
                if os.path.isdir(route):
                    ...
                    sub_directories.append(
                        route + "," + str(self.current_directory_number)
                    )
                    self.dirs.append(
                        {
                            "id": "EXTAD" + str(self.current_directory_number),
                            "name": folder,
                            "type": "directory",
                            "path": route,
                            "parent": "EXTAD" + str(parent),
                        }
                    )
                    self.current_directory_number += 1
                else:
                    ...
                    self.files.append(
                        {
                            "id": "EXTAF" + str(self.current_file_number),
                            "name": folder,
                            "path": route,
                            "type": file_type,
                            "parent": "EXTAD" + str(parent),
        ...
        if self.list_status is not False:
                    ...
                    try:
                        ...
                        if file_urls != [] and file_urls is not False:
                            for file_url in file_urls:
                                self.urls.append(
                                    {
                                        "id": "EXTAU" + str(self.current_url_number),
                                        "parent": file_id,
                                        "type": "url",
                                        "name": file_url,
                                    }
            ...
            for file in self.files:
                ...
                prepare_edge = (
                    ...
                    + '", to: "'
                    + file["id"]
                    + "\", color:{color:'#fff', highlight:'#89ff00'}},"
                )
    ...
    def copysource(self, result_directory):
        ...
        for file in self.files:
            if (
                ...
                file_path = file["path"]
                new_path = helper.fixpath(
                    result_directory + "/" + file["name"] + ".src"
                )
                file_name = file["name"]
                if os.path.isfile(file_path):
                    ...
                    if file["type"] == "js":
                        # Retire js scan
                        core.updatelog(
                            "Running retirejs vulnerability scan on: " + file_name
                        )
                        try:
                            ...
                            core.updatelog(
                                "Error {0} while running retirejs scan on {1}".format(
                                    str(e), file_name
                                )
                            )
                            rjs_scan = []
    ...
    def savereport(self):
        try:
            ...
            relative_result_path = result_directory.replace(
                core.reports_path, "<reports_path>"
            )
            reportindex = {
                "name": report["name"],
                "version": report["version"],
                "id": curid,
                "report_directory": relative_result_path,
...
def clearAllResults():
    ...
    for report in all_reports["reports"]:
        core.updatelog(
            "Deleting analysis #{0} - {1}".format(report["id"], report["name"])
        )
        report_dir = report["report_directory"].replace(
        ...
        if os.path.isdir(report_dir):
            try:
                ...
                core.updatelog(
                    "Analysis #{0} deleted successfully!".format(report["id"])
                )
            except Exception as e:
                core.updatelog(
                    "Something went wrong while deleting Report directory {0}. Error: {1}".format(
                        report_dir, str(e)
                    )
                )
                logging.error(traceback.format_exc())
            ...
            core.updatelog(
                "Report directory ({1}) of #{0} not found!".format(
                    report["id"], report_dir
                )
            )
    core.updatelog(
        "All individual result directories deleted! Updating report index..."
    )
    try:
        ...
        core.updatelog(
            "Something went wrong while updating {0}! Error: {1}".format(
                report_index, str(e)
            )
        )
        logging.error(traceback.format_exc())
...
def clearResult(result_id):
    ...
    if not os.path.isdir(analysis_dir):
        core.updatelog(
            "Could not find any directory with the given analysis ID: " + result_id
        )
    else:
        try:
            ...
            core.updatelog(
                "Something went wrong while deleting analysis directory: " + str(e)
            )
            logging.error(traceback.format_exc())
    ...
    if result_id not in str(reportids):
        core.updatelog(
            "No analysis with the id {0} in analysis index file".format(result_id)
        )
        # print(result_id)
        # print(str(reportids))
        return False
        ...
        core.updatelog(
            "Removed analysis {0} from index.. writing index to file".format(result_id)
        )
