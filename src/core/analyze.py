...
def analyze(ext_name, ext_type="local"):
    ...
    if (
        ext_name.endswith(".crx")
        or ext_name.endswith(".xpi")
        or ext_name.endswith(".zip")
        or ext_name.endswith(".tar")
        or ext_name.endswith(".gzip")
    ):
        """
            ...
            if os.path.exists(extract_dir):
                if os.path.exists(extract_dir + "_old"):
                    ...
                    try:
                        ...
                        core.updatelog(
                            "Error while deleting: {0} . Error: {1}".format(
                                extract_dir + "_old", str(e)
                            )
                        )
                        logging.error(traceback.format_exc())
                        return "error: Something went wrong while deleting old scan directory {0}".format(
                            extract_dir + "_old"
                        )

                new_name = os.path.basename(extract_dir) + "_old"
                core.updatelog(
                    "Renaming old extract directory {0} as {1}".format(
        ...
        for domain in set(domains):
            ...
            if virustotal.pub_vt != []:
                ...
                if not virustotal_report[0]:
                    core.updatelog(
                        "Error getting virustotal result... Error: "
                        + virustotal_report[1]
                    )
                    domain_vt = {
