...
class GetLocalExtensions:
    ...
    def extract_chromium_plugins(self, parent_dir):
        ...
        for extension_dir in extension_dirs:
            ...
            for ver in extension_vers:
                manifest_file = helper.fixpath(
                    extension_path + "/" + ver + "/manifest.json"
                )
                ...
                if ext_name:
                    ...
                    ret_list.append(
                        ext_name + "," + helper.fixpath(extension_path + "/" + ver)
                    )
    ...
    def googlechrome(self):
        ...
        if self.os == "windows":
            ...
            chrome_directory = helper.fixpath(
                self.user_directory + "/.config/google-chrome/Default/Extensions"
            )
        elif self.os == "osx":
            chrome_directory = helper.fixpath(
                self.user_directory
        ...
        if chrome_directory != "":
            if os.path.isdir(chrome_directory):
                core.updatelog(
                    "Found Google chrome extension directory: " + chrome_directory
                )
    ...
    def firefox(self):
        ...
        if self.os == "windows":
            ...
            if os.path.isdir(firefox_directory):
                ...
                if os.path.isfile(firefox_profile):
                    ...
                    try:
                        ...
                        core.updatelog(
                            "Found firefox profile path: " + default_profile_path
                        )
                    except Exception:
                        core.updatelog(
            ...
            firefox_directory = helper.fixpath(
                self.user_directory + "/.mozilla/firefox/"
            )
            if os.path.isdir(firefox_directory):
                # firfox installed
                firefox_profile = helper.fixpath(firefox_directory + "/profiles.ini")
                ...
                if os.path.join(firefox_extension_directory):
                    ...
                    for afile in unfiltered_files:
                        if afile.endswith(".xpi") and os.path.isfile(
                            os.path.join(firefox_extension_directory, afile)
                        ):
        ...
        if xpi_files != []:
            exta_firefox_list = os.path.join(
                firefox_extension_directory, "extanalysis.json"
            )
        ...
        if read_list["extensions"] != {}:
            ...
            for fext in read_list["extensions"]:
                prepare_to_insert = (
                    read_list["extensions"][fext]["name"]
                    + ","
                    + read_list["extensions"][fext]["file"]
                )
...
def analyzelocalfirefoxextension(path):
    if os.path.isfile(path) and path.endswith(".xpi"):
        ...
        if "error:" in analysis_status:
            core.updatelog(
                "Something went wrong while analysis... deleting temporary extract directory"
            )
        else:
        ...
        core.updatelog(
            "[analyzelocalfirefoxextension] Invalid local firefox extension path: "
            + path
        )
