    ...
    try:
        ...
        try:
            ...
            reader = maxminddb.open_database(
                helper.fixpath(core.path + "/db/geoip.mmdb")
            )
            ...
            core.updatelog(
                "Something went wrong while getting country from ip {0}! Error: {1}".format(
                    ip, str(e)
                )
            )
        ...
        core.updatelog(
            "To save time in future analysis; install maxminddb by: pip3 install maxminddb"
        )
