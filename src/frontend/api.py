    ...
    if query == "dlanalysis":
        ...
        for areport in reportids["reports"]:
            ...
            rd += (
                ...
                + "</td><td>"
                + report_date
                + '</td><td><button class="bttn-fill bttn-xs bttn-primary" onclick=viewResult(\''
                + report_id
                + '\')><i class="fas fa-eye"></i> View</button> <button class="bttn-fill bttn-xs bttn-danger" onclick=deleteResult(\''
                + report_id
        ...
        try:
            ...
            if browser == "googlechrome":
                ...
                if exts is not False and exts != [] and exts is not None:
                    ...
                    for ext in exts:
                        ...
                        return_html += (
                            "<tr><td>"
                            + ext_info[0]
                            + '</td><td><button class="bttn-fill bttn-xs bttn-success" onclick="analyzeLocalExtension(\''
                            + ext_info[1].replace("\\", "\\\\")
                ...
                if exts and len(exts) > 0:
                    ...
                    return "error: Something went wrong while getting local Vivaldi browser extensions! Check log for more information"
        ...
        try:
            ...
            for data in whois_result:
                ...
                if isinstance(whois_result[data], list):
                    for subdata in whois_result[data]:
                        whois_html += (
                            '<b style="color:#89ff00;">{0} : </b>{1}<br>'.format(
                                proper_data, subdata
                            )
                        )
                else:
            ...
            whois_html += "</div>"
            if whois_result:
                return "<center><h4>Whois Results For {0}</h4></center><br>{1}".format(
                    domain, whois_html
                )
            else:
                return (
        ...
        try:
            ...
            if geo_ip[0]:
                ...
                rethtml += "</div>"
                return "<center><h4>Geo-IP Lookup Results For {0}</h4></center><br>{1}".format(
                    ip_address, rethtml
                )
        ...
        try:
            ...
            if headers_status[0]:
                ...
                rethtml += "</div>"
                return "<center><h4>Showing HTTP Headers of: {0}</h4></center><br>{1}".format(
                    url, rethtml
                )
            else:
        ...
        try:
            ...
            if headers_status[0]:
                ...
                return "<center><h4>Source Code of: {0}</h4></center><br>{1}".format(
                    rurl, rethtml
                )
            else:
                return "error: " + headers_status[1]
