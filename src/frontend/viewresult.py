...
def view(analysis_id):
    ...
    try:
        ...
        if not analysis_info[0]:
            ...
            error_txt = (
                "Something went wrong while getting analysis info!<br>Error: "
                + analysis_info[1]
            )
            ...
            if os.path.isdir(result_directory):
                ...
                if all(
                    ...
                    if "firefox" in extension_type.lower():
                        extension_type = (
                            '<i class="fab fa-firefox"></i> ' + extension_type
                        )
                    elif "chrome" in extension_type.lower():
                        extension_type = (
                            '<i class="fab fa-chrome"></i> ' + extension_type
                        )
                    ...
                    if report_data["domains"] != []:
                        ...
                        for domain in report_data["domains"]:
                            ...
                            if os.path.isfile(domain_flag):
                                ...
                                flag_path = url_for(
                                    "static", filename="images/flags/unknown.png"
                                )
                            country_html = (
                                '<img src="{0}" class="country_flag"> {1}'.format(
                        ...
                        domains_table = (
                            '<h3 class="nothing"> No Domains Extracted! </h3>'
                        )
                    ...
                    for perm in report_data["permissions"]:
                        ...
                        perm_html = '<div class="accordion"><div class="accordion__item"><div class="accordion__question">{0} {1} <div class="risk-pill {4}">{4}</div></div><div class="accordion__answer">{2} <div class="warning"> {3} </div></div></div></div>'.format(
                            perm["badge"],
                            helper.escape(perm["name"]),
                            perm["description"],
                            (perm["warning"] if perm["warning"] != "na" else ""),
                            perm["risk"],
                        )
                    ...
                    for file_info in source_data:
                        ...
                        file_action = (
                            '<button class="bttn-fill bttn-xs bttn-primary" onclick="viewfile(\''
                            + analysis_id
                            + "', '"
                            + file_id
                            + '\')"><i class="fas fa-code"></i> View Source</button>'
                        )
                        ...
                        file_html = "<tr><td>{2} {0}</td><td>{1}</td><td>{4}</td><td>{3}</td></tr>".format(
                            file_name, rel_path, file_type, file_action, file_size
                        )
                    ...
                    if (
                        report_data["ipv4_addresses"] == []
                        and report_data["ipv6_addresses"] == []
                    ):
                        ips_table = (
                            '<h3 class="nothing">No IPv4 or IPv6 addresses found!</h3>'
                        )
                    else:
                        ...
                        for comment in report_data["comments"]:
                            comments_table += (
                                "<tr><td>{0}</td><td>{1}</td></tr>".format(
                                    helper.escape(comment["comment"]), comment["file"]
                                )
                            )
